...
| **5c-1 — maxRingBeats re-strike** ✅ `fa61a07` | `maxRingBeats` param (planning only). A note whose buffered duration exceeds the limit is **re-articulated** every that-many beats (`pendingRestrikes` scheduled forward in `drainRestrikes`; the real note-off still releases it) — piano tone decays, and an 8-beat tie is neither idiomatic nor what a pianist plays. Logged `re-strike <note> every N beats`. | planning | (processor-side; no `ocpn`) |
| **5c-2 — figuration recognition** | tremolo / repeated-note detection over the buffered groups → collapse to held notes + a marker; `arpeggioRespace`; `octaveMovePassages` (whole-voice octave shift for a phrase out of hand range). | planning | figure detection |
