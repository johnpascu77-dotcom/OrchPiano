...
| **5c-1 — maxRingBeats re-strike** ✅ `<p5c1>` | `maxRingBeats` param (planning only). A note whose buffered duration exceeds the limit is **re-articulated** every that-many beats (`pendingRestrikes` scheduled forward in `drainRestrikes`; the real note-off still releases it) — piano tone decays, and an 8-beat tie is neither idiomatic nor what a pianist plays. Logged `re-strike <note> every N beats`. | planning | (processor-side; no `ocpn`) |
